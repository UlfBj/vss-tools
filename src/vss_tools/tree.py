...
class VSSNode(Node):  # type: ignore[misc]
    ...
    def get_instance_nodes(self) -> tuple[VSSNode, ...]:
        return findall(
            self,
            filter_=lambda node: isinstance(node.data, (VSSDataBranch, VSSDataProcedure)) and node.data.instances,
        )
...
def expand_instance(
    ...
    for name in names:
        for root in roots:
            ...
            node = template.copy()
            node.name = name
            if isinstance(node.data, VSSDataProcedure):
                # HIM Service Rule Set "multiplexed microservice tree
                # structure": a node generated by expanding a *procedure's*
                # 'instances' directive represents one resource instance
                # (e.g. 'Row1', 'Row1.DriverSide'), not another microservice.
                # It must become a plain 'branch' so that only the original,
                # unexpanded procedure node keeps type 'procedure'; each
                # resource-instance branch then carries its own nested
                # 'Input'/'Output' iostructs (added below via
                # add_expanded_instance_children). 'nativeRate'/'timeToLive'
                # are procedure-only concepts and are dropped; they still
                # apply to the (singular) enclosing procedure, not to each
                # resource instance individually.
                branch_dict = node.data.model_dump(
                    mode="json",
                    exclude={"fqn", "instances", "nativeRate", "timeToLive"},
                )
                branch_dict["type"] = "branch"
                node.data = get_vss_raw(branch_dict, node.get_fqn())
